# Matrix(N, lambda) -> List(Vector(N, 1))
# @param ordered_pop sorted population
generate_mean_seq = function(ordered_pop) {
  n = ncol(ordered_pop)
  n:2 %>%
    purrr::map(\(index) {
      matrix(apply(ordered_pop[, 1:index], 1, mean))
    })
}

# List(Vector(N, 1)) -> Vector(N, 1)
# @param trun_mean_seq a seuqnce of truncated means

estimate_mean_correction = function(trun_mean_seq) {
  n = length(trun_mean_seq)
  centroid_matrix = purrr::reduce(trun_mean_seq, base::cbind)
  mlm = lm(t(as.data.frame(centroid_matrix)) ~ seq.int(n))
  estimation = mlm[["fitted.values"]][n, ]
  matrix(estimation)
}

# Matrix(N, lambda) -> Vector(N, 1)
get_mean_corr = . %>% generate_mean_seq %>% estimate_mean_correction

# @Example
# x0 = rep(100, 10); fn = \(x) { sum(x*x) }; lower = -100; upper = 100
# result = cma_es(x0, fn, lower = lower, upper = upper)
#
# names(result$diagnostic)
# result$diagnostic contains logged values, i.e:
# * result$diagnostic$bestVal 
# * result$diagnostic$mean_val
# * result$diagnostic$corr_mean_val

cma_es <- function(par, fn, ..., lower, upper, if_CMA = TRUE, control=list()) {
  norm <- function(x)
  ...
  log.mean_val <- controlParam("diag.mean_val", log.all)
  log.corr_mean_val <- controlParam("diag.corr_mean_val", log.all)
  ...
  if (log.mean_val) 
    mean_val.log = matrix(0, nrow = 0, ncol = 1)
  if (log.corr_mean_val)
    corr_mean_val.log = matrix(0, nrow = 0, ncol = 1)
  ...
  while (iter < maxiter) {
    ...
    # mean point
    mean_point = matrix(apply(vx, 1, mean))
    mean_value = apply(mean_point, 2, fn)

    # corrected mean point
    corr_mean_point = get_mean_corr(vx)
    corr_mean_value = apply(corr_mean_point, 2, fn)

    if (log.mean_val)
      mean_val.log = rbind(mean_val.log, mean_value)
    if (log.corr_mean_val)
      corr_mean_val.log = rbind(corr_mean_val.log, corr_mean_value)
  ...
  if (log.mean_val) log$mean_val = mean_val.log
  if (log.corr_mean_val) log$corr_mean_val = corr_mean_val.log
